...
unsigned char rftu_reciever(void)
{
	...
	while(1)
	{
		...
		switch(waiting)
		{
			...
			default: // Read new packet
				recvfrom(sd, &rftu_pkg_rcv, sizeof(rftu_pkg_rcv), 0, (struct sockaddr *)&sender_soc, sizeof(sender_soc));
				// check the commanders
				switch(rftu_pkg_rcv.cmd)
				{
					case (RFTU_CMD_INIT):
					if(receiving == NO)
				    {
				    	// Open file
				    	file_info = *((struct file_info_t *) &rftu_pkg_rcv.data);
				    	...
				    	if (file_fd < 0)
				    	{
				    		...
				    		rftu_pkg_send_cmd.cmd = RFTU_CMD_NOSPACE;
				    		sendto(sd, &rftu_pkg_send_cmd, sizeof(rftu_pkg_send_cmd) , 0 , (struct sockaddr *) &sender_soc, sizeof(sender_soc));

				    	}
				    	else
				    	{
				    		// Send command READY to sender
				    		rftu_pkg_send_cmd.cmd = RFTU_CMD_READY;
				    		rftu_pkg_send_cmd.id = rand();
				    		sendto(sd, &rftu_pkg_send_cmd, sizeof(rftu_pkg_send_cmd) , 0 , (struct sockaddr *) &sender_soc, sizeof(sender_soc));
